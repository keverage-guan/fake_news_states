...
instead of encoding on the fly. Mirrors the Fakeddit precompute step but is
text-only (Yelp has no image modality).

Output: for each split file <name>.jsonl, writes
...
Usage:
    python src/precompute_yelp_embeddings.py \
        --splits data/splits/yelp/*.jsonl \
...
def parse_args():
    ...
    p.add_argument("--splits", nargs="+", required=True,
                   help="Split .jsonl files (globs allowed).")
