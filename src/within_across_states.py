"""
src/within_across_state.py

Step 6: Within- vs. across-state generalisation test.

Inspired by Baldassano et al. (2017, Neuron): tests whether windows that
share the same HMM latent state generalise better to each other than
windows in different states, after controlling for temporal distance.

Inputs
------
  --decode_npz   data/hmm_hmm/final_decode_k6.npz   (state_seq, window_ids)
  --f1_npz       data/hmm_perf/k6/cross_window_f1.npz  (f1_matrix 35×35)
...
-------------
  ...
  3. Distance-conditioned label shuffle
                        ...
                        within/across labels 10 000×.  Statistic is the
...
def parse_args():
    ...
    p.add_argument("--decode_npz",      default="data/hmm_hmm/6way/final_decode_k6.npz",
                   help="HMM decode file from src/fit_hmm_decode.py")
    p.add_argument("--f1_npz",          default="data/hmm_perf/6way/cross_window_f1.npz",
                   help="Cross-window F1 matrix from src/merge_cross_window.py")
    p.add_argument("--output_dir",      default="data/hmm_within_across/6way/k6")
...
def load_inputs(decode_npz: str, f1_npz: str):
    ...
    window_ids = dec["window_ids"].astype(int)  # (N,)  e.g. [0..34]

    f1_data   = np.load(f1_npz, allow_pickle=True)
    f1_matrix = f1_data["f1_matrix"].astype(float)  # (N, N)

    # Per-seed cube (N, n_seeds, N) — present in updated output files.
    # Falls back to None so the script also runs on old files.
    ...
    print(f"Loaded {N} windows, k={len(np.unique(state_seq))} states, "
          f"n_seeds={n_seeds}")
    print(f"State sequence: {state_seq}")
    return state_seq, window_ids, f1_matrix, f1_per_seed_cube
...
def build_pair_table(state_seq: np.ndarray,
    ...
    expanded into up to n_seeds rows — one per seed — giving 5× more
    observations.  NaN seed slots (failed evals) are dropped.

    When f1_per_seed_cube is None (old files), falls back to window-mean F1.
    ...
    if f1_per_seed_cube is not None:
        ...
        # Fallback: window-mean F1 (original behaviour)
        for i in range(N):
            for j in range(N):
                if i == j:
                    continue
...
def distance_stratified_summary(df: pd.DataFrame, rng: np.random.Generator) -> pd.DataFrame:
    """
...
def distance_conditioned_gap(df: pd.DataFrame) -> float:
    ...
    At each lag d, gap_d = mean(within F1 at d) − mean(across F1 at d).
    The pooled statistic is the weighted mean of gap_d, weighted by the
    harmonic mean of n_within and n_across at each lag (gives lower weight
    to lags where one class is very sparse).

    This is the observed statistic for the distance-conditioned test.
    """
    gaps    = []
    weights = []
...
def distance_conditioned_shuffle_test(
    ...
    same_state labels while preserving their counts at that lag.  This
    destroys any association between state membership and F1 *within*
    each lag bin, while leaving the marginal distribution of F1 values
    at every lag completely intact.

    Because the shuffle is done within-lag, the null can never be inflated
    by the temporal proximity confound — it is the sharpest possible
    distance-controlled test.

    The test statistic is distance_conditioned_gap(df): the harmonic-
    mean-weighted average of per-lag within-vs-across gaps.

    Returns (null_gaps, observed_gap, p_value).
    p_value is one-sided: fraction of null gaps ≥ observed gap.
    """
    observed_gap = distance_conditioned_gap(df)

    # Pre-group by distance so we don't re-group inside the loop
    lag_groups = {
        d: sub.copy()
        for d, sub in df.groupby("distance")
        if sub["same_state"].sum() > 0          # need at least one within pair
        and (sub["same_state"] == 0).sum() > 0  # and at least one across pair
    }
...
# ---------------------------------------------------------------------------
# State-pair heatmap helper
# ---------------------------------------------------------------------------

def statepair_mean_f1(df: pd.DataFrame, k: int) -> np.ndarray:
    """
    Return (k, k) matrix where entry [s_i, s_j] is the mean F1 when
    training on windows in state s_i and testing on windows in state s_j.
    Diagonal = within-state mean.
    """
...
def plot_f1_heatmap_with_states(
    ...
    Adds coloured rectangles along both axes marking each window's state,
    and dashed grid lines at state boundaries so within-state blocks are
    visually obvious.
    """
    N          = len(window_ids)
    ...
    # Draw twice: a thick white halo first, then a thinner dark line on top,
    # so the boundary reads clearly against any cell colour in the RdYlGn map.
    boundaries = [i - 0.5 for i in range(1, N)
                  if state_seq[i] != state_seq[i - 1]]
    for b in boundaries:
        for spine in (
            dict(color="white",   lw=4.0, ls="-", alpha=1.0, zorder=3),
            dict(color="#1a1a1a", lw=1.8, ls="-", alpha=1.0, zorder=4),
        ):
            ax.axvline(b, **spine)
            ax.axhline(b, **spine)

    # ── State colour strips (top + left margins via inset axes) ───────────
    # Top strip  — test-window state
    ax_top = ax.inset_axes([0, 1.01, 1, 0.03], transform=ax.transAxes)
    ax_top.set_xlim(-0.5, N - 0.5)
    ax_top.set_ylim(0, 1)
    ax_top.axis("off")
    for j in range(N):
        ax_top.bar(j, 1, color=STATE_PALETTE[state_seq[j] % len(STATE_PALETTE)],
                   edgecolor="none", width=1.0)

    # Left strip — train-window state
    ax_left = ax.inset_axes([-0.04, 0, 0.03, 1], transform=ax.transAxes)
    ax_left.set_ylim(N - 0.5, -0.5)
    ax_left.set_xlim(0, 1)
    ax_left.axis("off")
    for i in range(N):
        ax_left.barh(i, 1, color=STATE_PALETTE[state_seq[i] % len(STATE_PALETTE)],
                     edgecolor="none", height=1.0)

    # ── Ticks & labels ────────────────────────────────────────────────────
    tick_labels = [f"W{w:03d}" for w in window_ids]
    ...
    ax.set_title(f"Cross-window macro F1  (HMM k={k} states overlaid)",
                 fontsize=12, pad=18)
    ...
    ax.legend(handles=legend_patches, loc="lower right",
              fontsize=8, ncol=k, framealpha=0.8,
              title="HMM state", title_fontsize=8)
...
def plot_f1_vs_distance(summary_df: pd.DataFrame, output_path: str):
    """Line plot: within and across-state mean F1 ± 95 % CI vs lag."""
...
def plot_statepair_heatmap(mat: np.ndarray, k: int, output_path: str):
    ...
    ax.set_xticklabels([f"State {s}" for s in range(k)], fontsize=10)
    ax.set_yticklabels([f"State {s}" for s in range(k)], fontsize=10)
...
def plot_state_timeline(state_seq: np.ndarray, summary_df: pd.DataFrame,
    ...
    ax0.legend(handles=legend_patches, loc="upper right",
               fontsize=8, ncol=k, framealpha=0.7)
...
def main():
    ...
    # ------------------------------------------------------------------
    print("=" * 60)
    print("Step 6: Within- vs. across-state analysis")
    print("=" * 60)
    state_seq, window_ids, f1_matrix, f1_per_seed_cube = load_inputs(
    ...
    # ------------------------------------------------------------------
    print(f"\nRunning distance-conditioned label shuffle (n={args.n_permutations:,}) ...")
    ...
    print(f"  p-value                           = {dc_p_value:.4f}  "
          f"({'significant' if dc_p_value < 0.05 else 'not significant'} at α=0.05)")
