"""
src/select_hmm_states.py

Step 4 (model selection only): find the optimal number of HMM states.

Three complementary criteria are evaluated for k in [k_min, k_max]:
  1. BIC    -- computed on all per-seed Z_scaled sequences (n_seeds × n_windows obs).
  2. AIC    -- same fit, lighter penalty: 2 * n_params instead of
               n_params * log(n_obs).  Tends to favour slightly larger k.
...
All criteria use multiple random initialisations per k for stability.
An adjusted rand index (ARI) matrix is produced to gauge Viterbi stability
across inits for every k.
...
Usage
-----
  python src/fit_hmm_select_states.py \
...
Requirements: hmmlearn, scikit-learn, numpy, matplotlib, pandas, tqdm
"""
...
def fit_on_sequences(seqs, k, n_iter, random_state, covariance_type="diag"):
    ...
    model   = make_hmm(k, n_iter, random_state, covariance_type)
    try:
        model.fit(X, lengths)
    except Exception:
        return None
...
# FIX 4: divide total log-likelihood by sequence length so held-out scores
# are per-observation and comparable across folds even when sequence lengths
# differ across seeds.
...
def _n_params(k, D, covariance_type):
    """Number of free parameters for a GaussianHMM."""
    cov_params = {
        "diag":      k * D,
        "full":      k * D * (D + 1) // 2,
        "tied":      D * (D + 1) // 2,
    ...
    # (k-1) initial state probs  +  k*(k-1) transition probs  +  k*D means  +  cov
...
def compute_bic(model, seqs):
    """BIC = -2 * LL + n_params * log(n_obs)  -- lower is better."""
    ...
    if not np.isfinite(ll):
        return np.inf
    return -2.0 * ll + _n_params(k, D, model.covariance_type) * np.log(n_obs)


def compute_aic(model, seqs):
    """AIC = -2 * LL + 2 * n_params  -- lower is better, lighter penalty than BIC."""
    k  = model.n_components
    D  = seqs[0].shape[1]
    ll = _score_all(model, seqs)
    if not np.isfinite(ll):
...
# ---------------------------------------------------------------------------
# BIC + AIC (full data) -- also returns fitted models for ARI reuse
# ---------------------------------------------------------------------------

# FIX 3: return the fitted models so run_ari_stability can reuse them
# instead of refitting identical random seeds a second time.
def run_bic_aic(seed_seqs, k_range, n_inits, n_iter, covariance_type):
    """
    Fit n_inits HMMs on all seeds for each k.
    Report best (lowest) BIC and AIC across inits.
    Returns bic_best, bic_all, aic_best, aic_all, and a dict of fitted
    models keyed by (ki, init) for reuse in the ARI phase.
    """
    all_seqs   = list(seed_seqs.values())
    bic_all    = np.full((len(k_range), n_inits), np.inf)
    aic_all    = np.full((len(k_range), n_inits), np.inf)
    # FIX 3: store every successfully fitted model
    fitted_models = {}
    ...
    for ki, k in pbar_k:
        ...
        for init in pbar_init:
            ...
            fitted_models[(ki, init)] = model   # FIX 3: cache for ARI phase
...
# ---------------------------------------------------------------------------
# LOO-CV (leave-one-seed-out)
# ---------------------------------------------------------------------------

# FIX 2: covariance_type is now an explicit parameter instead of a global.
def run_loo_cv(seed_seqs, k_range, n_inits, n_iter, covariance_type):
    """
    Hold out one seed at a time. Fit on the rest (best init by train LL).
    Score the held-out seed's sequence (per-observation log-likelihood).
    """
    seeds   = sorted(seed_seqs.keys())
    n_seeds = len(seeds)
    # FIX 1: initialize to np.nan (not -np.inf) so that failed folds are
    # excluded by np.nanmean/np.nanstd rather than silently corrupting
    ...
    pbar_k = tqdm(
        enumerate(k_range), total=len(k_range),
        desc="  LOO", unit="k", position=0, leave=True,
        bar_format="  {l_bar}{bar}| k {n_fmt}/{total_fmt} [{elapsed}<{remaining}]",
    )
    for ki, k in pbar_k:
        pbar_k.set_description(f"  LOO  k={k}")
        t0 = time.time()

        pbar_fold = tqdm(
            enumerate(seeds), total=n_seeds,
            desc=f"    fold", unit="fold", position=1, leave=False,
            bar_format="    {l_bar}{bar}| fold {n_fmt}/{total_fmt}",
        )
        for fi, held_seed in pbar_fold:
            pbar_fold.set_description(f"    fold seed_{held_seed} held out")
            ...
            pbar_init = tqdm(
                range(n_inits), total=n_inits,
                desc=f"      init", unit="init", position=2, leave=False,
                bar_format="      {l_bar}{bar}| {n_fmt}/{total_fmt}",
            )
            for init in pbar_init:
                model = fit_on_sequences(
                    ...
                    covariance_type=covariance_type,   # FIX 2
                )
                if model is None:
                    failed += 1
                    pbar_init.set_postfix({"last": "FAIL", "failed": failed})
                ...
                if train_ll > best_ll_train:
                    ...
                    best_model    = model
                pbar_init.set_postfix({
                    "best_train_ll": f"{best_ll_train:.2f}",
                    "failed":        failed,
                })
            pbar_init.close()

            if best_model is not None:
                # FIX 4: score_sequence now returns per-observation LL
                held_ll         = score_sequence(best_model, held_seq)
                # FIX 1: np.nan is already the default; only write a real value
                # when scoring succeeded (score_sequence returns np.nan on failure)
                loo_all[ki, fi] = held_ll
                pbar_fold.set_postfix({
                    "held_ll/obs":  f"{held_ll:.4f}",
                    "train_ll":     f"{best_ll_train:.3f}",
                })
            else:
                pbar_fold.set_postfix({"status": "ALL INITS FAILED"})
                # loo_all[ki, fi] remains np.nan -- correctly excluded below
        pbar_fold.close()

        # np.nanmean/np.nanstd now correctly ignore both failed folds (nan)
        # and never see any -inf entries.
        ...
        print(
            ...
            f"({n_valid}/{n_seeds} folds) | {elapsed(t0)}\n"
            f"        | per fold → {per_fold_str}"
        )

    loo_mean = np.nanmean(loo_all, axis=1)
    loo_std  = np.nanstd(loo_all,  axis=1)
    # If every fold failed for a given k, nanmean returns nan -- keep as nan
...
# ---------------------------------------------------------------------------
# ARI stability -- reuses models fitted in the BIC/AIC phase
# ---------------------------------------------------------------------------

# FIX 3: accepts fitted_models from run_bic_aic; no redundant refitting.
def run_ari_stability(seed_seqs, k_range, n_inits, n_iter, covariance_type,
                      fitted_models):
    """
    Decode each BIC/AIC-phase model with Viterbi and compute pairwise ARI.
    Models are reused from run_bic_aic (same random seeds, no extra EM runs).
    Higher ARI = more stable solution across random starts.
    """
    all_seqs = list(seed_seqs.values())
...
def plot_loo_per_fold(k_range, loo_all, output_path):
    ...
    # FIX 1: loo_all now contains nan for failures, so nanmean is correct
    loo_mean = np.nanmean(loo_all, axis=1)
...
def main(args):
    ...
    # FIX 5: feasibility check shown for both full-data and LOO (n_seeds-1)
    # phases so degenerate k-values are flagged before any fitting begins.
    ...
    print(f"\n  → Make your choice, then run src/fit_hmm_decode.py --k <chosen_k>")
    print("═" * 60)
